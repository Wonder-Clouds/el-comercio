...
interface DebtorTableProps<TData, TValue> {
...
export function DebtorTable<TData, TValue>({
  columns,
  data,
  page,
  pageSize,
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header, index) => (
                  <TableHead
                    key={header.id}
                    className={`px-6 py-4 text-sm font-semibold text-gray-700 bg-gray-50 border-b border-gray-200 ${
                      index === columns.length - 1 ? "no-print" : ""
                    }`}
                  >
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map((row, rowIndex) => (
                <TableRow
                  key={row.id}
                  className={`border-b border-gray-100 last:border-0 transition-colors duration-200 hover:bg-gray-50/70 ${
